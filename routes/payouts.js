...
// GET /api/payouts — list with optional filters (OPS + FINANCE)
router.get('/', authorize('OPS', 'FINANCE'), async (req, res) => {
  try {
    ...
    if (typeof req.query.vendor_id === 'string' && req.query.vendor_id) {
...
// POST /api/payouts — create draft payout (OPS only)
...
// GET /api/payouts/:id/audit — audit trail
router.get('/:id/audit', authorize('OPS', 'FINANCE'), async (req, res) => {
  try {
    ...
    const logs = await PayoutAudit.find({ payout_id: req.params.id })
...
// POST /api/payouts/:id/submit — Draft → Submitted (OPS only)
...
// POST /api/payouts/:id/approve — Submitted → Approved (FINANCE only)
...
// POST /api/payouts/:id/reject — Submitted → Rejected (FINANCE only)
